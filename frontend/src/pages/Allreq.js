                    ...
                    {data.map((item, index) => {
                        return (
                            <li className="table-row" onClick={() => navigate('pendingstudents', { state: { unqId: item._id } })} key={index}>
                                ...
                                {item.isVerified && <div className="col mcol-5" data-label="Status" style={{ color: "green" }}>Certified</div>}
                                {!item.isVerified && <div className="col mcol-5" data-label="Status" style={{ color: "red" }}>Pending</div>}
                            </li>
                        )
                    })}
